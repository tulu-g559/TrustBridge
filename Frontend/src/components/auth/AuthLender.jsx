...
const signUpSchema = z
  .object({
    ...
    lendingInterest: z.enum(["education", "health", "farming", "business"]),
...
export default function LenderAuthPage() {
  ...
  const signUpForm = useForm({
    ...
    defaultValues: {
      ...
      confirmPassword: "",
      panId: "",
      lendingInterest: undefined,
      agreement: false,
    },
  });

  // Replace the existing onSignIn and onSignUp functions
...
const onSignIn = async (values) => {
  ...
  try {
    ...
    // Check if user exists in lenders collection
    const userDoc = await getDoc(doc(db, "lenders", user.uid));
    if (!userDoc.exists()) {
      throw new Error("Account not found in lender database");
    }

    // Successfully signed in as lender
...
const onSignUp = async (values) => {
  ...
  try {
    ...
    await setDoc(doc(db, "lenders", userId), {
      ...userData,
        ...
        <Card className="border border-gray-800/50 bg-black/40 backdrop-blur-xl shadow-xl">
          <CardHeader className="space-y-4 pb-8 border-b border-gray-800/50">
            <CardTitle className="text-3xl font-bold text-center bg-gradient-to-r from-purple-400 via-pink-400 to-blue-400 bg-clip-text text-transparent">
              TrustBridge Lender Portal
            </CardTitle>
            <CardDescription className="text-center text-gray-400 text-lg">
              Empower communities through sustainable finance
            </CardDescription>
          </CardHeader>
                          ...
                          <FormField
                            ...
                            render={({ field }) => (
                              <FormItem>
                                ...
                                >
                                  <FormControl>
                                    <SelectTrigger className="bg-gray-900/30 text-gray-100 border-gray-700/50">
